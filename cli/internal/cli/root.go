// Package cli implements the paybridge developer CLI: cobra commands on
// top of two clients — github.com/very-good-labs/paybridge-go for every
...
const (
	envAPIKey  = "PAYBRIDGE_API_KEY"
	envBaseURL = "PAYBRIDGE_BASE_URL"
)

// newRootCmd wires the full command tree with the persistent flags:
//
//	--api-key   explicit API key, beating PAYBRIDGE_API_KEY and config
...
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		Use:   "paybridge",
		Short: "Developer CLI for the paybridge payment orchestration platform",
		Long: `Developer CLI for the paybridge payment orchestration platform.

Two credentials, never interchangeable:
  - an API key (Authorization: Bearer sk_...) — moves money: balance,
    charges, payouts, bank accounts. Get one via "paybridge login" +
    "paybridge keys create", or export PAYBRIDGE_API_KEY.
  - a dashboard session token — manages your account: whoami, keys.

Credentials resolve in precedence order: --api-key flag >
PAYBRIDGE_API_KEY environment variable > the value stored by
"paybridge login"/"paybridge configure" in the config file.`,
		SilenceUsage:      true,
		SilenceErrors:     true,
	...
	pf.StringVar(&g.apiKeyFlag, "api-key", "", "merchant API key (overrides PAYBRIDGE_API_KEY and stored config)")
	pf.StringVar(&g.baseURLFlag, "base-url", "", "merchant API base URL (overrides PAYBRIDGE_BASE_URL and stored config)")
...
func (r *resolved) apiBase() string {
	...
	return paybridge.DefaultBaseURL
}
...
func (r *resolved) requireAPIKey() (string, error) {
	...
	return "", fmt.Errorf("no API key configured — create one with %q, store one with %q, or export %s=<key>",
		"paybridge keys create", "paybridge configure --api-key sk_...", envAPIKey)
}
...
func (r *resolved) requireSession() (string, error) {
	if r.prof.SessionToken == "" {
		return "", fmt.Errorf("not logged in — run %q first", "paybridge login")
	}
