...
List/create/revoke use the dashboard session from "paybridge login".
...
func newKeysListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(keys) == 0 {
				fmt.Fprintln(os.Stdout, "No API keys yet — create one: paybridge keys create")
				return nil
			}
...
skip storing it (e.g. when minting for another machine).`,
		Example: `  paybridge keys create --name "laptop"
  paybridge keys create --name "staging box" --no-store`,
		RunE: func(cmd *cobra.Command, args []string) error {
			r, err := g.load()
			if err != nil {
				return err
...
func newKeysRevokeCmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: `  paybridge keys revoke 0d9a6c1e-...`,
		RunE: func(cmd *cobra.Command, args []string) error {
			r, err := g.load()
			if err != nil {
				return err
