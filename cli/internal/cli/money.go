...
// moneyClient builds the paybridge-go client for the resolved profile.
...
func (r *resolved) moneyClient() (*paybridge.Client, error) {
	key, err := r.requireAPIKey()
	if err != nil {
		return nil, err
	}
	return paybridge.NewClient(key, paybridge.WithBaseURL(r.apiBase())), nil
}
...
separate pending-settlement endpoint.`,
		Example: `  paybridge balance
  paybridge balance --currency IDR`,
...
func newBanksCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `List paybridge's active bank directory (GET /v1/banks): the banks
usable for virtual accounts and, for payouts, the bank_code values
accepted by "paybridge bank-accounts register". Unauthenticated on the
server; ordered by sort_order.`,
	...
	cmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if keyErr != nil {
				c = paybridge.NewClient("", paybridge.WithBaseURL(r.apiBase()))
			}
...
unit — IDR has no minor unit in paybridge, so 12500 means Rp12.500.
...
or one is generated for this invocation.`,
		Example: `  paybridge charges create --amount 100000
  paybridge charges create --amount 100000 --channel qris --description "Order 42"
  paybridge charges create --amount 100000 --channel virtual_account --va-bank BCA
  paybridge charges create --amount 100000 --metadata order=42 --metadata plan=pro`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if amount <= 0 {
			...
			if idemKey == "" {
				idemKey = paybridge.GenerateIdempotencyKey()
			}
			charge, err := c.CreateCharge(cmd.Context(), paybridge.CreateChargeParams{
				Amount:           amount,
				Currency:         currency,
				Description:      desc,
	...
	cmd.Flags().StringVar(&vaBank, "va-bank", "", "bank code for virtual_account (see paybridge banks list)")
...
provider a real charge would use, so the quote matches the bill.`,
		Example: `  paybridge charges calculate-fee --amount 100000
  paybridge charges calculate-fee --amount 100000 --channel qris
  paybridge charges calculate-fee --amount 100000 --channel virtual_account --va-bank BCA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if amount <= 0 {
				return fmt.Errorf("--amount must be a positive integer (minor unit)")
			}
			...
			quote, err := c.CalculateFee(cmd.Context(), paybridge.CalculateFeeParams{
				Amount:   amount,
				Currency: currency,
				Channel:  channel,
				VABank:   vaBank,
...
security window, then resumed automatically.`,
		Example: `  paybridge bank-accounts register --bank-code BCA --account-number 1234567890 --account-holder-name "Budi Santoso"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			acct, err := c.RegisterBankAccount(cmd.Context(), paybridge.RegisterBankAccountParams{
				BankCode:          bankCode,
	...
	cmd.Flags().StringVar(&bankCode, "bank-code", "", "bank code from paybridge banks list (required)")
...
automatically, no action needed.`,
		Example: `  paybridge payouts create --bank-account-id <uuid> --amount 20000`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if bankAcctID == "" {
				return fmt.Errorf("--bank-account-id is required (see paybridge bank-accounts get)")
			}
			if amount <= 0 {
				return fmt.Errorf("--amount must be a positive integer (minor unit)")
			}
			...
			p, err := c.CreatePayout(cmd.Context(), paybridge.CreatePayoutParams{
