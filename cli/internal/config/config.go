// Package config stores the paybridge CLI's local state: a single JSON
// file at $XDG_CONFIG_HOME/paybridge/config.json (defaulting to
// ~/.config/paybridge/config.json), mode 0600, holding per-profile
...
func Path() (string, error) {
	...
	return filepath.Join(dir, "paybridge", "config.json"), nil
}
